 ...
 * - right join
 * - full join
 * tìm tất cả comments của tấm hình số 3, cùng với username của người comment
 * đếm số comments của 1 tấm hình
 * tính trung bình mỗi tấm hình có bao nhiêu comments
 * tìm thằng user nào post hình và comment nhiều nhất
 * tìm tấm hình có nhiều comments nhất
 * tính trung bình có bao nhiêu ký tự mỗi comment
 * ai comment lên chính photo của họ?
 * */
...
/*
! giải thích cách hoạt động của join
* 1. tạo ra bảng tạm comments with users
* 2. đi qua từng row của bảng comments, và dùng user_id để match với từng row của bảng users
* 3. nếu tìm thấy id match, thì copy từ bảng users xuống bảng tạm comments with users
* 4. nếu có where thì chỉ giữ lại những row thỏa mãn điều kiện
* 5. xóa hết những column ko cần thiết, giữ lại cái muốn lấy

* Note:
* Thứ tự bảng giữa FROM và JOIN thường mang lại kết quả khác nhau
* Nếu 2 bảng khác nhau, có tên column giống nhau thì dùng tên bảng hoặc AS để lấy cho đúng
*/
...
/*
! Giải thích cách join 3 bảng: comments + photos + users
* 1. copy lấy hết row của bảng comments bỏ vào bảng tạm là comments with photos with users
* 2. đi qua từng row của bảng comments, và dùng photo_id để match với từng row của bảng photos
* 3. nếu tìm thấy id match, thì copy từ bảng photos xuống bảng tạm
* 4. đi qua từng row của bảng tạm, nếu comments.user_id = photos.user_id = users.id thì copy
* từ bảng users xuống bảng tạm, nếu ko bằng thì để NULL
* 5. đến bước này thì xét đến thể loại join
* - inner join: chỉ giữ lại những row có giá trị không NULL ở cả 3 bảng
* - left join: giữ lại tất cả row của bảng comments, nếu không tìm thấy match ở photos hoặc users thì để NULL
* - right join: giữ lại tất cả row của bảng photos, nếu không tìm thấy match ở comments hoặc users thì để NULL
* - full join: giữ lại tất cả row của cả 2 bảng, nếu không tìm thấy match thì để NULL
*/
