package javaoop.week03protectedinheritancethissuper;
 ...
 * bản chấp của kế thừa là copy, sau khi copy xong thì tạo ra 1 cái riêng của
 * class con (không có chuyện lớp con truy cập vào thuốc tính hay phương thức
 * của lớp cha
 * mà chỉ là truy cập đến phương thức hoặc thuộc tính của chính nó, và những
 * pt/tt đó được copy từ class Cha
 *
 * parent class - child class, super class - sub class, base class - derived
 * class
 * dùng kế thừa để ko bị dư code
 * java ko support đa kế thừa, chỉ hỗ trợ đơn kế thừa
 * 
 * Constructor
 * tạo data cho instance variable theo ý mình muốn ngay khi object được tạo, nếu
 * không java sẽ để giá trị mặc định cho các instance variable
 * contructor cũng giống method nhưng ko có return type, tên constructor giống
 * với tên Class
 * khi tạo object, thì constructor sẽ tự động được object call, mình ko cần call
 * để follow convention, thì khi muốn khởi tạo data, luôn khởi tạo trong
 * constructor, ko nên để trực tiếp ở instance variable
