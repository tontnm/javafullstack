package javaoop.week01classobject;
 ...
 * có nhiều methods thì phải maintain nhiều document, còn phải nghĩ ra nhiều
 * method name nữa
 * method overloading: 2 hay nhiều methods cùng tên trong cùng class (kể cả khi
 * dùng extends), khác số lượng param, khác param type
 * mỗi method sẽ có stack riêng của nó
 * 
 * his and super methods
 * mỗi object sẽ có 1 constructor mặc định, khi tạo object, constructor này sẽ
 * tự động được gọi
 * mỗi constructor sẽ có 1 super() mặc định, và super() method này sẽ tự động đc
 * gọi
 ...
 * 
 * method overriding
 * 2 methods giống tên nhau ở 2 class khác nhau, 1 class cha, 1 class con, thì
 * gọi là method overriding, khi tạo object con, thì xài method con
 * access modifer ở lớp con mà ít quyền hơn ở lớp cha thì báo lỗi
 * hàm static không được OR, nhưng vẫn OL bình thường
 * 
 * this and super keywords
 * nếu class A extends class B, Class A có biến a, Class B sẽ copy biến a của
 * Class A qua Class B
 * nếu class A extends class B, Class A có biến a, Class B cũng có biến a, khi
 * dùng a hoặc this.a sẽ lấy trước tiếp a của Class B, khi dùng super.a, thì sẽ
 * lấy biến a đã được copy từ Class A qua Class B
 * khi kiếm ở Class B, biến a và this.a, phù hợp, nó sẽ lấy ở Class B, còn khi
 * có từ khóa super, thì nó lấy thẳng ở Class A
